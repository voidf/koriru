...
def deathloop(is_study):
	...
	while True:
		try:
			...
			print('领取完成，正在等待次日启动')
			statusText.config(state=tk.NORMAL)
			statusText.insert(tk.END,'领取完成，正在等待次日启动\n')
			statusText.config(state=tk.DISABLED)
			...
			print('阻塞%d秒'%slpt)
			statusText.config(state=tk.NORMAL)
			statusText.insert(tk.END,'等待%d秒\n' % slpt)
			statusText.config(state=tk.DISABLED)
			time.sleep(slpt)
			dosign(init_ck)
			continue
...
def restoreck():
	...
	try:
		...
		msrt=tk.Tk()
		msgb=tk.Message(msrt,text='找不到上次保存的ck文件,不在本程序同一目录下').pack()
		
def saveck():
	stvt=ent.get('1.0',tk.END)[:-1]
	ckf=open(resource_path('LastCK.ck'),'w')
	ckf.write(stvt)
	ckf.close()
	subm1()
...
rt=tk.Tk()
rt.title("请输入您的cookie:")
rt.geometry('400x250')
ent=tk.Text(rt,width=50,height=15)
ent.pack()
bt=tk.Button(rt,text='确定',width=15,height=2,command=subm1).pack(side='left')
restoreckButton=tk.Button(rt,text='使用保存的ck',width=15,height=2,command=restoreck).pack(side='left')
saveckButton=tk.Button(rt,text='输入并保存ck',width=15,height=2,command=saveck).pack(side='left')
rt.mainloop()