...
def make_req(ck):
	...
	with open(sys.path[0]+'/'+'temp.png','wb') as f:
		f.write(pngf)
	return open(sys.path[0]+'/'+'temp.png','rb')
	...
	if swi==1 and chk==1:
		...
		for studying in range(4):
			...
			for name_ctr in range(10):
				if studying==2:
					break
				if os.path.exists(sys.path[0]+'/学习/'+dislist[studying]+'-'+str(name_ctr)+'.gif'):
					continue
				else:
					print(targ)
					targ=targ.crop(scan_dark_pixs(targ))
					targ.save(sys.path[0]+'/学习/'+dislist[studying]+'-'+str(name_ctr)+'.gif')
...
while True:
	...
	try:
		...
		for debugi in range(20):
			if os.path.exists(sys.path[0]+'/BUG_%d' % debugi):
				continue
			else:
